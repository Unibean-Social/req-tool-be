...
async def audit_item(
    ...
    # Closed item must have CloseReason
    if item.status in TERMINAL_STATUSES:
        ...
        checks.append({
            "rule": "CLOSE_REASON_REQUIRED",
            "pass": has_reason,
            "detail": "Closed item has a CloseReason record" if has_reason else "Closed item missing CloseReason",
        })
    else:
        checks.append({"rule": "CLOSE_REASON_REQUIRED", "pass": True, "detail": "Item is not closed"})

    # Label completeness
    labels = list(item.labels or [])
    missing_prefixes = [p for p in _LABEL_PREFIXES if not any(lbl.startswith(p) for lbl in labels)]
    checks.append({
        "rule": "LABEL_COMPLETE",
    ...
    if item_type == ItemType.epic:
        ...
        checks.append({
            ...
            "pass": ac_count > 0,
            "detail": f"{ac_count} acceptance criteria" if ac_count > 0 else "No acceptance criteria — at least 1 required",
        })

    elif item_type == ItemType.feature:
        from app.models.nfr import nfr_feature_links
        nfr_count = await db.scalar(
            select(func.count()).select_from(nfr_feature_links).where(
                nfr_feature_links.c.feature_id == item_id
            )
        ) or 0
        has_nfr = nfr_count > 0
        checks.append({
            "rule": "NFR_LINK_REQUIRED",
            "pass": has_nfr,
            "detail": f"{nfr_count} NFR(s) linked" if has_nfr else "No NFRs linked to this feature",
        })
